...
GO
CREATE PROCEDURE [dbo].[LoadEEHPythonAnalysis] as
...
BEGIN
	...
	--Record01 (game data)
	INSERT INTO EEHGames (WhiteLast, WhiteFirst, WhiteElo, BlackLast, BlackFirst, BlackElo, ECO, GameDate, Tournament, RoundNum, Result, Moves, CorrFlag)
	SELECT
	...
	--Record02 (game moves)
	INSERT INTO EEHMoves (GameID, MoveNumber, Color, IsTheory, IsTablebase, Move,
	...
	--update misc columns
	UPDATE EEHMoves
	SET Move_Rank = dbo.EEHMovePlayedRanking(MoveID),
		CandidateMoves = dbo.CountEEHMovesCandidates(MoveID)
	WHERE TotalMoves IS NULL

	EXEC UpdateEEHAvgCandidateError
	...
	UPDATE EEHMoves SET TotalMoves = dbo.CountEEHMoves(MoveID) WHERE TotalMoves IS NULL
	...
	VALUES ('3', @vfilename, GETDATE(), @ctr)
