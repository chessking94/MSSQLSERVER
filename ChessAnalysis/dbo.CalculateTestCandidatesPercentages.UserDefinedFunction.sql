...
GO
CREATE FUNCTION [dbo].[CalculateTestCandidatesPercentages] (@MoveID int, @MinCPDiff float, @MaxCPDiff float)
RETURNS decimal(6,4)
...
BEGIN
	...
	DECLARE @eval float
	SET @eval = (SELECT CAST((CASE WHEN T1_Eval LIKE '#+%' THEN 100.00 WHEN T1_Eval LIKE '#-%' THEN -100.00 ELSE T1_Eval END) AS float) FROM CheatTestMoves WHERE MoveID = @MoveID)

	DECLARE @candidatecount int
	DECLARE @ct int
	SET @candidatecount = (SELECT CandidateMoves FROM CheatTestMoves WHERE MoveID = @MoveID)
