...
GO
CREATE function [dbo].[fn_USCFNormCalculator] (@Tournament varchar(50))
RETURNS varchar(30)
...
BEGIN
	...
	SET @gamecount = (SELECT COUNT(GameID) FROM EEHGames WHERE Tournament = @Tournament)
	IF @gamecount = 0
	BEGIN
		SET @norm = 'No tournament with that name!'
		RETURN @norm
	END
	ELSE IF @gamecount <= 3
	BEGIN
	...
	SET @score = (SELECT SUM(CASE WHEN Result = 0.5 THEN 0.5 WHEN ((WhiteLast = 'Hunt' AND WhiteFirst = 'Ethan' AND Result = 1.0) OR ((BlackLast = 'Hunt' AND BlackFirst = 'Ethan' AND Result = 0.0))) THEN 1.0 ELSE 0.0 END) FROM EEHGames WHERE Tournament = @Tournament)
	SET @ctr = 1
	...
	WHILE @ctr <= @rounds
	BEGIN
		SET @elo = (SELECT TOP 1 ISNULL(a.Rating, 0) FROM (SELECT RoundNum, CASE WHEN WhiteLast = 'Hunt' AND WhiteFirst = 'Ethan' THEN BlackElo ELSE WhiteElo END AS 'Rating' FROM EEHGames WHERE Tournament = @Tournament) a WHERE a.RoundNum = @ctr)
