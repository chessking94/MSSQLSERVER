...
GO
CREATE PROCEDURE [dbo].[LoadControlPythonAnalysis] as
...
BEGIN
	...
	--Record02 (game moves)
	INSERT INTO ControlMoves (GameID, MoveNumber, Color, IsTheory, IsTablebase, Move,
	...
	--update misc columns
	UPDATE ControlMoves
	SET Move_Rank = dbo.ControlMovePlayedRanking(MoveID),
		CandidateMoves = dbo.CountControlMovesCandidates(MoveID)
	WHERE TotalMoves IS NULL

	EXEC UpdateControlAvgCandidateError

	UPDATE ControlMoves
	SET	StDevCandidate = dbo.SDControlCandidateMoves(MoveID),
		PcntCandidatesWithin10CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.00, 0.09),
		PcntCandidatesWithin20CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.10, 0.19),
		PcntCandidatesWithin30CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.20, 0.29),
		PcntCandidatesWithin40CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.30, 0.39),
		PcntCandidatesWithin50CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.40, 0.49),
		PcntCandidatesWithin60CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.50, 0.59),
		PcntCandidatesWithin70CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.60, 0.69),
		PcntCandidatesWithin80CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.70, 0.79),
		PcntCandidatesWithin90CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.80, 0.89),
		PcntCandidatesWithin100CP = dbo.CalculateControlCandidatesPercentages(MoveID, 0.90, 1.00),
	...
	VALUES ('1', @vfilename, GETDATE(), @ctr)
