...
GO
CREATE procedure [dbo].[LoadTestingPythonAnalysis] as

EXEC ClearCheatTestTables
...
BEGIN
	...
	--Record01 (game data)
	INSERT INTO CheatTestGames (WhiteLast, WhiteFirst, WhiteElo, BlackLast, BlackFirst, BlackElo, ECO, GameDate, Tournament, RoundNum, Result, Moves, CorrFlag)
	SELECT
	...
	--Record02 (game moves)
	INSERT INTO CheatTestMoves (GameID, MoveNumber, Color, IsTheory, IsTablebase, Move,
	...
	--update misc columns
	UPDATE CheatTestMoves
	SET Move_Rank = dbo.TestMovePlayedRanking(MoveID),
		CandidateMoves = dbo.CountTestMovesCandidates(MoveID)
	WHERE TotalMoves IS NULL

	EXEC UpdateTestAvgCandidateError
	...
	UPDATE CheatTestMoves SET TotalMoves = dbo.CountTestMoves(MoveID) WHERE TotalMoves IS NULL
	...
	Values ('2', @vfilename, GETDATE(), @ctr)
